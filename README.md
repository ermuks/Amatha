...
현재 버전은 `Directory.Build.props` 기준 **1.5.1**입니다.
...
```
Amaranth10API/
├── Directory.Build.props     앱·설치기 공통 버전 (1.5.1)
...
로그인 전에는 왼쪽 패널이 숨겨집니다. 대시보드에 들어가면 햄버거 메뉴가 나타납니다. 창 아래 상태 줄 오른쪽은 `ver 1.5.1` (`AppVersion.FooterLabel`), 왼쪽은 새 버전이 있을 때만 “새 버전이 있습니다. (v…)” 링크입니다.
...
시작할 때와 이후 **1시간마다** GitHub 버전을 봅니다. 햄버거 메뉴의 **업데이트 확인**은 바로 한 번 더 봅니다. 새 버전이 있으면 상태 줄 왼쪽 링크가 보이고, 버튼으로 확인했는데 이미 최신이면 안내만 띄웁니다. 링크를 누르면 확인 창에 **지금 버전 → 새 버전**과 GitHub 커밋 요약을 보여 준 뒤, 설치기를 받아 `runas`로 실행하고 본 프로그램을 종료합니다. 관리자 확인을 취소하거나 다운로드가 실패하면 링크를 되돌립니다. 백그라운드 확인이 실패해도 앱은 그대로 씁니다.
...
- **반려·취소가 아닌** 출장신청서가 덮는 날 (캘린더에 아직 없어도 포함, 결재 진행 중도 포함). 제목에 `해외`가 있으면 해외출장으로 표시
...
| 진행 | `진행` | `30` | 신청한 날은 예정 날짜 집합에 넣고, 카드에 종결 대기 문구 |
...
| 파일 | 역할 |
| --- | --- |
| `Directory.Build.props` | 공통 버전 1.5.1 |
